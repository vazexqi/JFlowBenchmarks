...
public class FloatArray2DSIFT {
    ...
    /**
     * number of orientation histograms per axis of the feature descriptor square
     */
     ...
     * 
     * @param src image having a generating gaussian kernel of initial_sigma img must be a 2d-array
     ...
     * @param max_size maximal size of an octave to be taken into account Use this to save memory
     *            and procesing time, if processing higher resolutions is not necessary.
     */
    public void init(
            FloatArray2D src,
            int steps,
    ...
    /**
     * sample the scaled and rotated gradients in a region around the features location, the regions
     * size is defined by ( FEATURE_DESCRIPTOR_SIZE * 4 )^2 ( 4x4 subregions )
     * 
     * @param c candidate 0=>x, 1=>y, 2=>scale index
     * @param o octave index
     * @param octave_sigma sigma of the corresponding gaussian kernel with respect to the scale
     *            octave
    ...
    /**
     * assign orientation to the given candidate, if more than one orientations found, duplicate the
     * feature for each orientation
     * <p/>
    ...
    void processCandidate(
        ...
        /**
         * interpolate orientation estimate the offset from center of the parabolic extremum of the
         * taylor series through env[1], derivatives via central difference and laplace
         */
        ...
        /**
         * check if there is another significant orientation ( > 80% max ) if there is one,
         * duplicate the feature and
         */
        for (int i= 0; i < ORIENTATION_BINS; ++i) {
            if (i != max_i &&
                ...
                /**
                 * interpolate orientation estimate the offset from center of the parabolic extremum
                 * of the taylor series through env[1], derivatives via central difference and
                 * laplace
                 */
