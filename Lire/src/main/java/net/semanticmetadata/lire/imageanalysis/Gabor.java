...
public class Gabor implements LireFeature {
    ...
    public double getDistance(double[] targetFeatureVector, double[] queryFeatureVector) {
        ...
        for (int m= 0; m < M; m++) {
            for (int n= 0; n < N; n++) {
                distance+= Math.sqrt(Math.pow(queryFeatureVector[m * 2 * N + n * 2] - targetFeatureVector[m * 2 * N + n * 2], 2)
                        + Math.pow(queryFeatureVector[m * 2 * N + n * 2 + 1] - targetFeatureVector[m * 2 * N + n * 2 + 1], 2));
            }
        }
            ...
            for (int n= 0; n < N; n++) {
                ...
                for (int x= S; x < image.getWidth(); x++) {
                    for (int y= T; y < image.getHeight(); y++) {
                        magnitudesForVariance[m][n]+= Math.pow(Math.sqrt(Math.pow(this.gaborWavelet[x - S][y - T][m][n][0], 2) + Math.pow(this.gaborWavelet[x - S][y - T][m][n][1], 2))
