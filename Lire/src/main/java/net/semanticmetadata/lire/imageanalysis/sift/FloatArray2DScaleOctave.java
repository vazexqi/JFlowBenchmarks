 ...
 * <p/>
 * This class is optimized for the Difference Of Gaussian detector used in David Lowe's
 * SIFT-algorithm \citep{Loew04}.
 * <p/>
 * The scale space itself consists of an arbitrary number of octaves. This number is implicitly
 * defined by the minimal image size {@link #MIN_SIZE}. Octaves contain overlapping scales of the
 * scalespace. Thus it is possible to execute several operations that depend on adjacent scales
 * within one octave.
 * <p/>
 * BibTeX:
 * 
...
public class FloatArray2DScaleOctave {
     ...
     * <p/>
     * the first member is the sigma of the gaussian kernel that is assumed to be the generating
     * kernel of the first gaussian image instance of the octave
     */
    public float[] SIGMA;
...
//	}
    ...
    /**
     * sigma of gaussian kernels required to create the corresponding gaussian image instances from
     * the first one
     */
    private float[] SIGMA_DIFF;

    /**
     * 1D gaussian kernels required to create the corresponding gaussian image instances from the
     * first one
     */
    ...
    /**
     * get the gradients of the corresponding gaussian image, generates it on demand, if not yet
     * available.
     * 
     * @param i index will not be checked for efficiency reasons, so take care that it is within a
     *            valid range
     ...
     * Constructor
     * 
     * @param img image being the first gaussian instance of the scale octave img must be a 2d-array
     *            of float values in range [0.0f, ..., 1.0f]
     ...
     * <p/>
     * Use this method for the partial creation of an octaved scale space without creating each
     * scale octave. Like proposed by Lowe \citep{Lowe04}, you can use this image to build the next
     * scale octave. Taking every second pixel of this image, you get a gaussian image with
    ...
    /**
     * downsample {@link src} by simply using every second pixel into {@link dst}
     * <p/>
     * For efficiency reasons, the dimensions of {@link dst} are not checked, that is, you have to
     * take care, that dst.width == src.width / 2 + src.width % 2 && dst.height == src.height / 2 +
     * src.height % 2 .
     * 
     ...
     * <p/>
     * For efficiency reasons, the dimensions of {@link dst} are not checked, that is, you have to
     * take care, that src.width == dst.width / 2 + dst.width % 2 && src.height == dst.height / 2 +
     * dst.height % 2 .
     * 
     * @param src the source image
     * @param dst destination image
