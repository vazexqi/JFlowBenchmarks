...
/**
 * Utility class for serialization issues. Created by: Mathias Lux, mathias@juggle.at Date:
 * 19.03.2010 Time: 14:58:26
 */
public class SerializationUtils {
    /**
     * Converts a byte array with 4 elements to an int. Used to put ints into a byte[] payload in a
     * convenient and fast way by shifting without using streams (which is kind of slow). <br/>
    ...
    /**
     * Converts an int to a byte array with 4 elements. Used to put ints into a byte[] payload in a
     * convenient and fast way by shifting without using streams (which is kind of slow). <br/>
     * Taken from http://www.daniweb.com/code/snippet216874.html
     * 
    ...
    /**
     * Converts a float to a byte array with 4 elements. Used to put floats into a byte[] payload in
     * a convenient and fast way by shifting without using streams (which is kind of slow). Use
    ...
    /**
     * Converts a byte array with 4 elements to a float. Used to put floats into a byte[] payload in
     * a convenient and fast way by shifting without using streams (which is kind of slow). Use
    ...
    /**
     * Converts a double to a byte array with 4 elements. Used to put doubles into a byte[] payload
     * in a convenient and fast way by shifting without using streams (which is kind of slow). Use
     * {@link net.semanticmetadata.lire.utils.SerializationUtils#toDouble(byte[])} to decode. Note
     * that there is a loss in precision as the double is converted to a float in the course of
     * conversion.
     * 
    ...
    /**
     * Converts a byte array with 4 elements to a double. Used to put doubles into a byte[] payload
     * in a convenient and fast way by shifting without using streams (which is kind of slow). Use
     * {@link net.semanticmetadata.lire.utils.SerializationUtils#toBytes(double)} to encode. Note
     * that there is a loss in precision as the double is converted to a float in the course of
     * conversion.
     * 
     * @param data the input byte array
